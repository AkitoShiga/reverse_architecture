...
function discToMessage(disc) {
    return disc === DARK ? '黒' : '白'
}

function showNextDiscMessage(nextDisc) {
    if(nextDisc) {
        nextDiscMessageElement.innerText = `次の石は${discToMessage(nextDisc)}です`
    } else {
...
function warningMessage(previousDisc, nextDisc, winnerDisc) {
    if(nextDisc !== null) {
        if(previousDisc === nextDisc) {
            ...
            return `${discToMessage(skipped)}の番をスキップします`
        } else {
            return null
        }
    } else {
        if(winnerDisc === WINENR_DRAW) {
            return '引き分けです'
        } else {
            return `${discToMessage(winnerDisc)}の勝ちです`
        }
    }
}
